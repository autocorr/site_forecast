...
class PlotlyFigureBase:
    ...
    def add_quantile_traces(
        ...
        for n_s in antenna_counts:
            ...
            ser = (
                self.series[
                    self.series.index.get_level_values("site").isin(self.site_names)
                ]
                .groupby(level="date")
                .quantile(min(q, 1.0))
            )
...
class PlotlyFigureVlbaStack(PlotlyFigureVlbaBase):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.layout = get_multi_site_stack_layout(
            n_rows=self.n_rows, limits=self.limits
        )
    ...
    def add_sun_rise_set(self, opacity=0.2):
        for i, name in enumerate(self.site_names):
            ...
            patches = get_sun_rise_set_patches(
                rises, sets, self.y_lim, yaxis=f"y{i + 1}", opacity=opacity
            )
...
class PlotlyFigureVlbaAgg(PlotlyFigureVlbaBase):
    ...
    def add_sun_rise_set(self, opacity=0.075):
        for name in self.site_names:
            ...
            for i in (1, 2):
                patches = get_sun_rise_set_patches(
                    rises, sets, self.y_lim, yaxis=f"y{i}", opacity=opacity
                )
                self.data.append(patches)
