...
10. **Cleanup.** Remove deprecated aliases: `SITE_LAT`/`SITE_LON`, the `Forecast.weather` legacy attributes, `df.attrs["has_bad"]`, and the raw `.df`/`.okay`/`save_data` members on the former query classes. Add the stub `GBT` profile. Delete `QueryBase` once nothing inherits from it, and delete or archive `query/nomads_cutout.py` per the decision in Section 10.
...
- **`query/nomads_cutout.py`.** ~500 lines, currently imported by nothing in the operational pipeline. Decide during migration: delete it, or port it as an `Op` when a consumer exists. It should not be migrated speculatively.
